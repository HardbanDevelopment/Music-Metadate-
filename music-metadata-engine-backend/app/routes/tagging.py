        ...
        try:
                ...
                try:
                    ...
                    for item in parsed_tipl:
                        if not isinstance(item, dict) or "role" not in item or "name" not in item:
                            raise ValueError("Each TIPL item must be a dictionary with 'role' and 'name'.")
            ...
            if audio is not None:
                if isinstance(audio, ID3):
                    ...
                    # Write ISRC
                    if isrc:
                        audio.delall("TXXX:ISRC") # Remove existing ISRC to avoid duplicates
                    ...
                    if parsed_tipl:
                        ...
                        for frame_id in list(audio.keys()):
                            if frame_id.startswith("TXXX:") and frame_id.split(":")[1] in [item['role'] for item in parsed_tipl]:
                    ...
                    if parsed_tipl:
                        ...
                        for item in parsed_tipl:
                            audio.add_tag('involvedpeople', f"{item['role']}={item['name']}") # Using custom field
