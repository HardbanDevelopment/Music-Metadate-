...
async def analyze_audio_track(file: UploadFile = File(...)):
    ...
    if not MIRService.is_available():
         ...
         return JSONResponse(
            status_code=503, 
            content={"error": "MIR libraries (Librosa) not active on server. Check installation."}
        )
...
async def separate_track_vocals(
    ...
    if not SeparationService.is_available():
         return JSONResponse(status_code=503, content={"error": "Demucs/Torch not active."})
    ...
    try:
        ...
        # Separate
        result = await SeparationService.separate_vocals(temp_input_path, temp_output_dir)
        ...
        if vocals_file and os.path.exists(vocals_file):
            ...
            return JSONResponse(status_code=500, content={"error": "Separation completed but files not found."})
