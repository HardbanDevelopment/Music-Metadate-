...
try:
    ...
    from mutagen.id3 import ID3, TIT2, TPE1, TALB, TCON, TDRC, COMM, USLT, TPUB, TCOP, TCOM, TEXT
...
class MIRService:
    ...
    async def analyze_audio(file_path: str):
        ...
        try:
            ...
            danceability = float(np.mean(pulse)) # Normalize logic would be needed for 0-1
    ...
    def write_metadata(file_path: str, metadata: dict):
        ...
        try:
            ...
            if ext in ['.mp3', '.wav']:
                # Basic
                if 'title' in metadata: audio.tags.add(TIT2(encoding=3, text=metadata['title']))
                if 'artist' in metadata: audio.tags.add(TPE1(encoding=3, text=metadata['artist']))
                if 'album' in metadata: audio.tags.add(TALB(encoding=3, text=metadata['album']))
                if 'genre' in metadata: audio.tags.add(TCON(encoding=3, text=metadata['genre']))
                if 'year' in metadata: audio.tags.add(TDRC(encoding=3, text=str(metadata['year'])))
                
                # Extended (Requested by user)
                if 'publisher' in metadata: audio.tags.add(TPUB(encoding=3, text=metadata['publisher']))
                if 'label' in metadata: audio.tags.add(TPUB(encoding=3, text=metadata['label'])) # Fallback if same
                if 'copyright' in metadata: audio.tags.add(TCOP(encoding=3, text=metadata['copyright']))
                if 'composer' in metadata: audio.tags.add(TCOM(encoding=3, text=metadata['composer']))
                if 'lyricist' in metadata: audio.tags.add(TEXT(encoding=3, text=metadata['lyricist']))
            ...
            if ext == '.flac':
                ...
                if 'copyright' in metadata: audio['copyright'] = metadata['copyright']
                if 'publisher' in metadata: audio['publisher'] = metadata['publisher'] or metadata.get('label')
                if 'lyrics' in metadata: audio['lyrics'] = metadata['lyrics']
