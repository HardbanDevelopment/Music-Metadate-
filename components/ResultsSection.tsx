...
import { refineMetadataField, generateCoverArt, generateMarketingContent } from '../services/geminiService';
...
import UnifiedMetadataCard from './results/UnifiedMetadataCard';
import CreativeSuiteSidebar from './results/CreativeSuiteSidebar';
import Button from './Button';
import AnalysisSourceCard from './results/AnalysisSourceCard';
import WaveformDisplay from './results/WaveformDisplay';
import IdentificationCard from './results/IdentificationCard';
import CopyrightCard from './results/CopyrightCard';
...
const ResultsSection: React.FC<ResultsSectionProps> = ({ isLoading, error, results, onNewAnalysis, showToast, onUpdateResults, currentAnalysis, uploadedFile, theme, onBackToBatch }) => {
  ...
  const getRefinementInstruction = (field: keyof Metadata): string => {
    switch (field) {
      ...
      case 'trackDescription': return "Create a rich, evocative description highlighting emotions and atmosphere.";
  ...
  const handleRefine = async (field: keyof Metadata) => {
    ...
    try {
      ...
      showToast(`Field refined successfully!`, 'success');
    } catch (err) {
  ...
  const handleGenerateArt = async () => {
    ...
    setCoverArtUrl(null);
    try {
      const imageUrl = await generateCoverArt(results);
      setCoverArtUrl(imageUrl);
      showToast("Album cover generated successfully!", 'success');
    } catch (err) {
  ...
  const handleEmbedMetadata = async () => {
    if (!uploadedFile) {
      showToast("No source file available for tagging.", 'error');
      return;
    }
    if (!editedResults) return;
