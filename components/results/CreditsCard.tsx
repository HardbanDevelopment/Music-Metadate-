...
const CreditField: React.FC<{
    ...
    onRefine: (field: keyof Metadata) => void;
    refiningField: keyof Metadata | null;
    onGenerate?: () => void; // Opcjonalny generator
...
const GeneratorSettings: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    const handleReset = () => {
        if(confirm("Czy na pewno zresetować liczniki sekwencji do 1?")) {
            resetSequences();
        ...
        <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-lg mb-4 border border-slate-200 dark:border-slate-700 animate-fade-in">
            <div className="flex justify-between items-center mb-3">
                <h4 className="text-sm font-bold text-light-text dark:text-dark-text flex items-center gap-2">
                    <Settings className="w-4 h-4" /> Ustawienia Generatorów
                </h4>
                <button onClick={handleReset} className="text-xs text-red-500 hover:underline flex items-center gap-1">
                    <RefreshCw className="w-3 h-3" /> Resetuj liczniki
                </button>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <label className="block text-xs font-bold text-slate-500 mb-1">Prefiks ISRC (Kraj-Rejestrant)</label>
                    <input 
                        type="text" 
                        value={config.isrcPrefix} 
                        onChange={(e) => setConfig({...config, isrcPrefix: e.target.value.toUpperCase()})}
                        placeholder="np. PL-A12"
                        className="w-full p-2 text-sm rounded border border-slate-300 dark:border-slate-600 dark:bg-slate-900"
                    />
                    <p className="text-[10px] text-slate-400 mt-1">Format: CC-XXX (np. PL-S1Z)</p>
                </div>
                <div>
                    <label className="block text-xs font-bold text-slate-500 mb-1">Prefiks Katalogowy</label>
                    <input 
                        type="text" 
                        ...
                        className="w-full p-2 text-sm rounded border border-slate-300 dark:border-slate-600 dark:bg-slate-900"
                    />
                    <p className="text-[10px] text-slate-400 mt-1">Wynik: {config.catPrefix}-{new Date().getFullYear()}-001</p>
                </div>
            </div>
            
            <div className="flex justify-end gap-2">
                <Button onClick={onClose} size="sm" variant="secondary">Anuluj</Button>
                <Button onClick={handleSave} size="sm" variant="primary">Zapisz Ustawienia</Button>
            </div>
        </div>
    );
};
        ...
        <Card>
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4 mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-lg font-bold text-light-text dark:text-dark-text">Prawa i Twórcy</h3>
                        <p className="text-sm text-slate-500 dark:text-slate-400">Metadane wydawnicze i prawne.</p>
                    </div>
                </div>
                {isEditing && (
                    <button 
