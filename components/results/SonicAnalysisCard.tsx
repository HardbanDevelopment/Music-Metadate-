            ...
            <div className="flex-grow space-y-2">
                {isAnalyzing ? (
                     <div className="flex flex-col items-center justify-center h-32 text-slate-400">
                        ...
                        <span className="text-xs font-semibold">Pełna analiza AI...</span>
                     </div>
                ) : (!results || results.length === 0) ? (
                    <div className="flex flex-col items-center justify-center h-32 text-slate-400">
                         <p className="text-xs italic">{emptyMessage || "Brak wyników."}</p>
                         <button onClick={onRetry} className="mt-2 text-xs text-accent-violet hover:underline">Analizuj</button>
                    </div>
                ) : (
                            ...
                            <div className="absolute inset-0 flex items-center justify-between px-3">
                                ...
                                <div className="flex items-center gap-2 z-10">
                                     ...
                                     <div className="hidden group-hover:flex gap-1 bg-white dark:bg-slate-800 pl-2 shadow-sm rounded">
                                        {type === 'inst' ? (
                                            <Tooltip text="Ustaw jako instrument główny">
                                             ...
                                             <Tooltip text="Ustaw jako gatunek główny">
            ...
            {!isAnalyzing && results && results.length > 0 && (
                <div className="mt-auto pt-2 border-t border-slate-100 dark:border-slate-800">
                    <Button onClick={onRetry} disabled={isAnalyzing} size="sm" variant="secondary" className="w-full text-xs py-1.5 h-8">
                        <RefreshCw className="w-3 h-3" /> Ponów analizę
                    </Button>
                </div>
            )}
        </div>
...
const SonicAnalysisCard: React.FC<SonicAnalysisCardProps> = ({ metadata, file, onUpdateMetadata, showToast }) => {
    ...
    try {
        ...
        if (highConfidenceInstruments.length > 0) {
            ...
            if (addedCount > 0) {
                ...
                showToast(`Automatycznie dodano ${addedCount} pewnych instrumentów.`, 'success');
            }
        }

        showToast("Analiza soniczna zakończona sukcesem.", 'success');
    } catch (err) {
  ...
  const handleApplyResult = (value: string, type: 'main' | 'add' | 'main_single', category: 'genre' | 'mood' | 'inst') => {
    ...
    if (category === 'genre') {
        if (type === 'main') {
            ...
            showToast(`Ustawiono główny gatunek: ${formatted}`, 'success');
        } else {
             ...
             if (!current.some(g => g.toLowerCase() === formatted.toLowerCase())) {
                 ...
                 showToast(`Dodano gatunek: ${formatted}`, 'success');
             }
        }
        ...
        if (!current.some(m => m.toLowerCase() === formatted.toLowerCase())) {
             ...
             showToast(`Dodano nastrój: ${formatted}`, 'success');
        }
    } else if (category === 'inst') {
        if (type === 'main_single') {
            onUpdateMetadata({ mainInstrument: formatted });
            showToast(`Ustawiono instrument główny: ${formatted}`, 'success');
        } else {
            ...
            if (!current.some(i => i.toLowerCase() === formatted.toLowerCase())) {
                 ...
                 showToast(`Dodano instrument: ${formatted}`, 'success');
            }
        }
    }
  };
  ...
  const handleAddTop3 = (results: HFClassificationResult[], category: 'genre' | 'inst') => {
      ...
      if (addedCount > 0) {
          showToast(`Dodano ${addedCount} nowych tagów.`, 'success');
      } else {
          showToast("Te tagi są już dodane.", 'info');
      }
  };
    ...
    <Card>
      <div className="flex items-center gap-3 border-b border-slate-200 dark:border-slate-800 pb-4 mb-6">
        ...
        <div className="flex-grow">
          <h3 className="text-lg font-bold text-light-text dark:text-dark-text">Inżynieria Dźwięku & AI</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400">Zaawansowany DSP i Klasyfikacja Gemini.</p>
        </div>
        ...
        <div className="bg-slate-50 dark:bg-slate-900/30 rounded-lg p-4 border border-slate-100 dark:border-slate-800 flex flex-col h-full">
             <div className="flex items-center justify-between mb-4 text-accent-violet">
                <div className="flex items-center gap-2">
                    ...
                    <h4 className="font-bold text-sm uppercase tracking-wide">Analiza Techniczna</h4>
                </div>
             ...
             <div className="grid grid-cols-2 gap-3 mb-4">
                ...
                <div className="bg-white dark:bg-slate-800 p-3 rounded shadow-sm text-center">
                    <div className="text-[10px] text-slate-500 uppercase mb-1">Tonacja</div>
             ...
             {dspData && (
                 <div className="space-y-4 mt-2 pt-2 border-t border-slate-200 dark:border-slate-800">
                    <div>
                        <h5 className="text-xs font-bold text-slate-500 mb-2">Korelacja Fazy</h5>
                    ...
                    {dspData.spectralBalance && (
                        <div>
                             <div className="flex justify-between items-center mb-2">
                                <h5 className="text-xs font-bold text-slate-500">Balans Spektralny</h5>
        ...
        <div className="bg-slate-50 dark:bg-slate-900/30 rounded-lg p-4 border border-slate-100 dark:border-slate-800 relative h-full flex flex-col">
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2 text-pink-500">
                    ...
                    <h4 className="font-bold text-sm uppercase tracking-wide">Klasyfikacja AI (Gemini)</h4>
                </div>
            </div>
            ...
            <div className="flex-grow overflow-y-auto custom-scrollbar min-h-[200px] flex flex-col">
                {activeTab === 'genre' && (
                    <ClassificationBlock 
                        ...
                        emptyMessage="Wybierz 'Analizuj', aby pobrać pełny profil soniczny."
                        type="genre"
                    />
                )}
