...
const EditableVocalStyle: React.FC<{
    ...
    if (!value || value.gender === 'None') {
        return <p className="text-sm text-slate-500 dark:text-slate-400 italic">Brak wokalu</p>;
    }

    const fields: { key: keyof VocalStyle; label: string }[] = [
        { key: 'gender', label: 'Płeć' },
        { key: 'timbre', label: 'Barwa' },
        { key: 'delivery', label: 'Wykonanie' },
        { key: 'emotionalTone', label: 'Emocje' },
    ];
        ...
        <Card>
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex justify-between items-center">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-lg font-bold text-light-text dark:text-dark-text text-left">Analiza Pro</h3>
                        <p className="text-sm text-slate-500 dark:text-slate-400 text-left">Szczegółowe dane techniczne i kreatywne.</p>
                    </div>
                </div>
            ...
            {isOpen && (
                <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800 space-y-4 animate-fade-in">
                    <ProField label="Styl wokalny" field="vocalStyle" {...{refiningField, onRefine}}>
                        <EditableVocalStyle 
                            isEditing={isEditing} 
                            value={vocalStyle} 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <ProField label="Instrumentarium" field="instrumentation" {...{refiningField, onRefine}}>
                        ...
                        <ProField label="Zastosowania" field="useCases" {...{refiningField, onRefine}}>
                            <EditableProField isEditing={isEditing} value={useCases} onChange={(v) => onFieldUpdate('useCases', v)} field="useCases" />
                        </ProField>
                    </div>
                    <ProField label="Struktura" field="structure" {...{refiningField, onRefine}}>
